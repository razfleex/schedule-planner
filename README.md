...
Составитель должен иметь возможность:
создавать учебное занятие;
просматривать данные занятия;
изменять занятие;
удалять занятие.
...
Составитель должен иметь возможность:
просматривать всё расписание;
фильтровать занятия по учебной группе;
фильтровать занятия по преподавателю;
фильтровать занятия по аудитории;
фильтровать занятия по дню недели.
Студент должен иметь возможность просматривать расписание учебной группы, с которой он связан.
Преподаватель должен иметь возможность просматривать занятия, в которых он указан преподавателем.
...
слой представления и API — принимает запросы и возвращает результаты;
...
создание и изменение преподавателей, групп, дисциплин и аудиторий;
создание, изменение и удаление занятий;
проверку корректности входных данных;
обнаружение конфликта преподавателя;
обнаружение конфликта учебной группы;
обнаружение конфликта аудитории;
случай, когда одно занятие начинается в момент окончания другого;
доступность действий в зависимости от роли пользователя.

### 4.3 Ограничения системы

В основной версии программы:
расписание составляется Составителем вручную;
расписание формируется для типовой учебной недели;
...
используются только три роли: Составитель, Студент и Преподаватель;
создание дополнительных ролей не предусматривается;
настройка произвольных наборов прав пользователей не предусматривается.
...
хранение данных о преподавателях, учебных группах, дисциплинах, аудиториях и занятиях;
создание, изменение и удаление преподавателей, учебных групп, дисциплин и аудиторий Составителем;
создание, изменение и удаление учебных занятий Составителем;
проверку занятий на временные конфликты;
просмотр расписания в зависимости от роли пользователя;
пользовательский веб-интерфейс;
серверную часть приложения;
хранение данных в базе данных;
автоматизированную проверку основных сценариев работы;
документирование способа запуска и использования программы.
...
получить актуальную версию проекта из репозитория GitHub;
подготовить файл переменных окружения на основе примера, если он требуется;
выполнить команду docker compose up --build;
дождаться запуска компонентов приложения;
открыть веб-приложение в браузере по адресу, указанному в README.md.
...
проект получают из репозитория;
подготавливают необходимые переменные окружения;
выполняют docker compose up --build;
убеждаются, что необходимые компоненты приложения успешно запущены;
открывают приложение в браузере;
выполняют основные сценарии из раздела 7.2;
останавливают приложение;
повторно запускают приложение;
проверяют сохранность ранее созданных данных.

### 7.4 Критерии приемки

Работа считается принятой, если:
приложение запускается через Docker Compose без изменения исходного кода;
основные функции, описанные в разделе 4 README.md, выполняются в соответствии с требованиями;
занятия с временными конфликтами не сохраняются;
корректные занятия сохраняются;
соприкасающиеся по времени занятия не считаются конфликтующими;
Студент и Преподаватель получают только предусмотренные для их ролей представления расписания;
Студент и Преподаватель не могут изменять данные расписания;
данные сохраняются после перезапуска приложения;
при недопустимой операции система сообщает причину отказа.